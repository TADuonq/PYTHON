...
root = Tk()
c = Canvas(root, width = canvas_wight, height = canvas_height,
           background = 'deep sky blue')
c.create_rectangle(-5, canvas_height - 100, canvas_wight + 5, 
...
catcher_width = 100
catcher_height = 100
catcher_start_x = canvas_wight / 2 - catcher_width / 2
...
lives_remaining = 3
lives_text = c.create_text(canvas_wight - 10, 10, anchor = 'ne',
...
def check_catch():
    (catcher_x, catcher_y, cather_x2, catcher_y2) = c.coords(catcher)
    for egg in eggs:
        (egg_x, egg_y, egg_x2, egg_y2) = c.coords(egg)
        if catcher_x < egg_x and egg_x2 < catcher_y2 and catcher_y2 - egg_y2 < 40:
            eggs.remove(egg)
            c.delete(egg)
            increase_scrore(egg_score)
    root.after(100, check_catch)

def increase_scrore(points):
